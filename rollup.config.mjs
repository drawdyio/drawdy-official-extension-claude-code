...
function webviewHtml() {
    return {
        ...
        async writeBundle() {
            ...
            writeFileSync(
                join(pkgRoot, "src", "webview-html.ts"),
                `// Generated by rollup.config.mjs. Do not edit by hand.\n` +
                    `// The extension publishing service builds src/index.ts with esbuild and\n` +
                    `// cannot run bundler plugins, so this module is committed to the repo.\n` +
                    `export const WEBVIEW_HTML = ${JSON.stringify(html)};\n`
            );
        },
    };
}
...
const driver = {
    ...
    plugins: [typescript(), packDrawdyx()],
};
